...
public class JwtUtil implements Serializable {
    ...
    private String secret = "dinningdaddysecret";
    ...
    public String validateAndRefreshJWT(String token) {
        try {
            long diff = getExpirationDateFromToken(token).getTime() - new Date().getTime();
            if (diff > 0) {
                if (diff < 300000) {
